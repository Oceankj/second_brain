...
sequenceDiagram
    ...
    Note over Client,Server: 接著進入登入、授權與 token 兌換流程
```
...
| 登入與授權頁 | `https://memory.example.com/oauth/authorize` |
...
- Discovery 宣告 PKCE `S256` 與 callback issuer identification；不宣告尚未支援的 CIMD。
